...
Where necessary, there are secondary, machine-specific config files that are sourced into the main ones. Where not possible, there are simply two different files, one .pc and other .laptop, which are in turn symlinked to the .config directory for the programs to read them. There shouldn't be many of these, though, and all of the rest remains the same between the two different machines.
